  ...
  return (
    <div className="min-h-screen bg-gray-100 dark:bg-gray-900 text-gray-900 dark:text-white transition-colors duration-300">
      ...
      <main className="max-w-[1800px] mx-auto px-6 py-6">
            {/* Summary Cards - TailAdmin style small widgets (using SummaryCard wrapper) */}
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
              <SummaryCard
                title="Métricas disponibles"
                value={Array.isArray(metrics) ? metrics.length : 0}
                variant="blue"
                icon={(
              ...
              <SummaryCard
                title="Inclinación"
                value={analysis?.evaluation?.inclination?.value != null ? `${analysis.evaluation.inclination.value.toFixed(1)}%` : '--'}
                variant={analysis?.evaluation?.inclination?.value != null ? (analysis.evaluation.inclination.value > 0 ? 'green' : analysis.evaluation.inclination.value < 0 ? 'red' : 'yellow') : 'gray'}
                icon={(
                  <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
