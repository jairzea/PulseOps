...
export const ConfirmModal: React.FC<ConfirmModalProps> = ({
    ...
    return (
        ...
        >
            {/* Backdrop con blur */}
            <div
                className={`absolute inset-0 bg-black/60 backdrop-blur-sm transition-all duration-300 ${isAnimating ? 'opacity-100' : 'opacity-0'
                    }`}
            />
            ...
            >
                ...
                <div className="relative p-6">
                    ...
                    <div className="flex justify-center mb-6">
                        <div
                            className={`w-24 h-24 rounded-full ${styles.icon} flex items-center justify-center transition-all duration-500 ${isAnimating ? 'scale-100 rotate-0' : 'scale-0 rotate-180'
                                }`}
                        >
                            {animation || isLoading ? (
                    ...
                    {!isLoading && (
                        <div
                            className={`flex gap-3 transition-all duration-500 delay-150 ${isAnimating ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-2'
                                }`}
                        >
                            <button
                                onClick={onClose}
